        ...
        <Dropdown isOpen={dropdown} toggle={toggleMenu} direction="down" size="md" className="dd-profile">
          <DropdownToggle color='transparent' className='dd-toggle'>
            <div className="d-inline-flex">
              ...
              <img className='rounded-circle' width='30px' height='30px' src={user.photo} alt='discord-pic' />
            </div>
          </DropdownToggle>
          <DropdownMenu >
            <DropdownItem onClick={() => navigate('/stats')}><i className='fa-solid fa-user me-3'></i>My Profile</DropdownItem>
            <DropdownItem onClick={removeToken}><i className='fa-solid fa-right-from-bracket me-3 text-danger'></i>Logout</DropdownItem>
          </DropdownMenu>
        </Dropdown>
        :
        <img role='button' className="discordButton" onClick={getToken} src={DiscordButton} alt='discord-button' />
      }
    </StyledDiscordButton>
  );
}
